...
if(((program.key1 || program.key2) && program.x509) ||
   (program.key1 || program.key2) && (program.thumbprint1 || program.thumbprint2)) {
...
if(program.x509) {
  if (program.thumbprint1 || program.thumbprint2) {
    ...
    generateCert(info, createDevice);
  }
...
function generateCert(deviceInfo, createDeviceFunc) {
  var pem = require('pem');
  var fs = require('fs');
